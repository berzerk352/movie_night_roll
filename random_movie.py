...
def main(participants):
    ...
    if participants == "ALL":
        participants = reduce(lambda accum, val: 
                              accum + [val[1].strip()] if val[1].strip() not in accum and val[1].strip() not in season_roster
                              else accum, results, [])
    ...
    if participants == []:
        ...
        participants = reduce(lambda accum, val: 
                              accum + [val[1].strip()] if val[1].strip() not in accum
                              else accum, results, [])
    ...
    config['random_movie']['season_roster'] = config['random_movie']['season_roster'] + f",{random_user}"
